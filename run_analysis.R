...
## reading x_train set . First we paste the fie dir wirh the name of the file
...
# reading the features.txt file to extract the indexes we are interested in 
tmp <- read.table("features.txt",header= FALSE, sep=" ", nrows=561)
features <- tbl_df(tmp[ grepl("mean()",tmp$V2) | grepl("std()",tmp$V2),]) # we are interested only in those with text including "mean()" or "std()" 

 ## Now we extract  the columns we are interested in from the merged total_x data set -> to a custom table ( my_total_x).
my_total_x <- select(total_x, features$V1)

## now we  name the columns with the names of the features taken from features$V2 
my_total_x <- setNames(my_total_x, features$V2) 

## Reading the activity_labels.txt to extract the names of the activites. 
## This file it's assumed to be present on the base dir  (Data-Cleaning-Course-Project folder)
...
my_total_y <- total_y %>% 
                ...
                select(activity) # with select() we'll get rid of the the old V1 col we inherited from the original total_y

# finally merging together the three data sets, putting "subject" and activity as the firts two columns.
